    ...
    configureServer(server: any) {
      ...
      server.middlewares.use('/api/report', async (req: any, res: any) => {
        ...
        req.on('end', async () => {
          ...
          sections.push(`**Build / context**\n\n- humanSide: \`${body.humanSide || 'unknown'}\` (AI: \`${body.aiSide || 'unknown'}\`)\n- userAgent: \`${body.userAgent || ''}\`\n- canEncodeState: \`${body.canEncodeState}\`\n- build: \`${body.build || 'unknown'}\`\n- gameId: \`${body.gameId || 'unknown'}\`\n- AI config: mcts=\`${aiFlag(body.mctsPolicy)}\` mctsRebel=\`${aiFlag(body.mctsRebel)}\` planner=\`${aiFlag(body.empirePlanner)}\` huntOccupy=\`${aiFlag(body.huntOccupy)}\` sabotageClear=\`${aiFlag(body.sabotageClear)}\`\n- timestamp: \`${body.timestamp || ''}\``);
