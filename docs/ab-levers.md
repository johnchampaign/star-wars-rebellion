...
| `SWR_BUNKERS` | **ON (was OFF)** | contingent — **verdict REVERSED on re-test** | 2026-08-15, 600 RoE (300/arm, paired seeds) | **ACCEPTED.** The 2026-08-06 rejection (−2.5pp) explicitly asked for a re-test against a stronger opponent, because the Rebel of that era "enters the Death Star's system in 3.3% of games and has never destroyed a DSUC" — the Bunker's protection had nothing to protect against. That precondition is gone: the modern bench sees the Rebel destroy **21 Death Stars and 5 DSUCs per 300 games**. Re-measured: bunkers deployed 288 → 470, **bunkers reaching the Death Star's system 22 → 111**, **Death Star Plans attempts BLOCKED 0 → 11**, Death Stars lost **21 → 17**. Win rate 36.0 → 38.0 (+2.0pp, CI [−5.7,+9.7]) — still inside noise, but the sign flipped from −2.5pp, the direction the old entry predicted. Note the failure was PLACEMENT, not construction: with the lever off the Empire still built ~1 bunker/game, but only 22 of 288 (7.6%) ever reached the station the unit exists for — the rest sat on ordinary planets as immobile structures with no attack. Reported by jocke01: "it still never uses the shield bunker to try and protect the death star." Guarded by test-shield-bunker-guards-death-star. |
