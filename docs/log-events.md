...
| `action-card-play` | — | `{"cardId":"early-promotion","leaderId":null,"systemId":null,"timing":"Immediate"}` | src/engine/phases.ts |
| `action-card-unknown` | — | `(not seen in corpus or sample game)` | src/engine/phases.ts |
| `activate-system` | Leader activates a system; orders = source systems, unitsMoved = total units. | `{"leaderId":"general-tagge","targetSystemId":"bothawui","orders":1}` | src/engine/phases.ts |
...
| `build-queue` | Unit added to the build queue. | `{"typeId":"rebel-trooper","slot":1,"sourceSystemId":"kashyyyk"}` | src/engine/mechanics.ts |
...
| `capture-leader` | Leader captured (ring: captured/carbonite). | `{"leaderId":"general-rieekan","ring":"captured","systemId":"kashyyyk"}` | src/engine/mechanics.ts |
| `capture-operative-pick` | — | `{"leaderId":"han-solo"}` | src/engine/phases.ts |
| `captured-leader-moved` | Empire | Escorted captive relocation (#595, RAW RR "Captured Leaders"): Imperial units moving out of a captive's system brought the captive along. Payload: leaderId, fromSystemId, toSystemId. | src/engine/phases.ts |
...
| `choice-request` | — | `{"kind":"PlayImmediateActionCard","candidates":["early-promotion"]}` | src/engine/cinematicTactics.ts, src/engine/combat.ts, src/engine/mechanics.ts, src/engine/objectives.ts, src/engine/phases.ts |
...
| `cinematic-prevent-applied` | — | `{"theater":"ground","round":2,"red":0,"black":1,"special":0}` | src/engine/combat.ts |
...
| `cinematic-tactic-play` | — | `{"cardId":"cin-empire-space-tractor-beam","ability":"primary","theater":"space","queued":"end-of-round-capture` | src/engine/cinematicTactics.ts, src/engine/combat.ts |
...
| `combat-begin` | Combat starts. A base assault also writes the base-assault snapshot. | `{"systemId":"kashyyyk","attackerSide":"Empire","cinematic":true}` | src/engine/combat.ts |
...
| `combat-end` | Combat over (rounds fought, winner). | `{"systemId":"kashyyyk","rounds":3,"winner":"Empire"}` | src/engine/combat.ts |
...
| `deploy` | One built unit deployed to a system (id + typeId). | `{"typeId":"rebel-trooper","systemId":"kashyyyk","unit":"u1000068"}` | src/engine/mechanics.ts |
...
| `destroy-unit` | One unit destroyed (id + typeId + where + cause). | `{"unit":"u1000071","typeId":"x-wing","systemId":"kashyyyk","cause":"combat"}` | src/engine/mechanics.ts |
...
| `draw-mission` | — | `{"count":2,"missionIds":["establish-trade-relations","base-defenses"]}` | src/engine/mechanics.ts |
| `draw-objective` | — | `{"count":1,"objectiveIds":["defensive-position-1"]}` | src/engine/mechanics.ts |
| `draw-probe` | — | `{"count":4,"probeIds":["probe-kashyyyk","probe-alderaan","probe-ryloth","probe-geonosis"]}` | src/engine/mechanics.ts |
...
| `gain-loyalty` | Loyalty gained at a system. | `{"systemId":"cato-neimoidia","newLoyalty":"rebel"}` | src/engine/mechanics.ts |
...
| `loyalty-already` | — | `{"systemId":"cato-neimoidia","loyalty":"rebel"}` | src/engine/mechanics.ts |
...
| `mission-discard` | — | `{"missionId":"regional-aid"}` | src/engine/combat.ts, src/engine/phases.ts |
| `mission-hand-trim` | — | `{"missionId":"message-from-high-command"}` | src/engine/phases.ts |
| `mission-return-to-hand` | — | `{"missionId":"rapid-mobilization","onFail":false}` | src/engine/combat.ts, src/engine/phases.ts |
| `mission-roll` | Contested mission dice resolution. | `{"missionId":"regional-aid","skill":"diplomacy","attacker":{"dice":3,"successes":4,"portrait":0,"landoBonus":0` | src/engine/phases.ts |
| `mission-unopposed` | Mission auto-succeeded unopposed. | `{"missionId":"build-alliance","result":"auto-success"}` | src/engine/phases.ts |
| `move-unit` | One unit moved (self-contained: id + typeId + from/to). | `{"unit":"s101106","from":"rodia","to":"bothawui"}` | src/engine/mechanics.ts |
...
| `pick-rebel-base` | Rebel base location chosen at setup. | `{"systemId":"utapau"}` | src/engine/phases.ts |
| `place-leader` | — | `{"leaderId":"general-rieekan","systemId":"rebel-base-space"}` | src/engine/mechanics.ts |
...
| `rapid-mobilization-probe-draw` | — | `{"count":4,"twoLeaders":false,"drawnProbeIds":["probe-kashyyyk","probe-alderaan","probe-ryloth","probe-geonosi` | src/engine/phases.ts |
...
| `recruit-leader` | Leader recruited. | `{"leaderId":"cassian-andor","cardId":"he-means-well"}` | src/engine/phases.ts |
| `recruit-pick-resolved` | — | `{"kept":"he-means-well","bottomed":["an-old-friend"]}` | src/engine/phases.ts |
...
| `remove-loyalty` | — | `{"systemId":"corellia"}` | src/engine/mechanics.ts |
...
| `reveal-mission` | Mission revealed at a target system. | `{"missionId":"rapid-mobilization","targetSystemId":"rebel-base-space","isAttempt":false}` | src/engine/phases.ts |
| `ring-attach` | — | `{"leaderId":"general-rieekan","ring":"k2so"}` | src/engine/mechanics.ts |
...
| `setup` | Game seed + starting loyalty draw. The seed here is meta.seed in v2. | `{"seed":143036853,"rebelLoyalty":["mon-calamari","bothawui","kashyyyk"],"imperialLoyalty":["mygeeto","mustafar` | src/engine/setup.ts |
...
| `subjugated` | System subjugated (Empire). | `{"systemId":"bothawui"}` | src/engine/mechanics.ts |
