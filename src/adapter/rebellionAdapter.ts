...
function dispatch(G: GameState, a: RebellionAction, actor: Side): Result {
  switch (a.kind) {
    ...
    case 'resolveAmbitionsOfPowerOffer':         return phases.resolveAmbitionsOfPowerOffer(G, a.accept);
    case 'resolveArmCardProbePick':              return phases.resolveArmCardProbePick(G, a.probeId);
    case 'resolveBehindEnemyLinesUnits':         return phases.resolveBehindEnemyLinesUnits(G, a.unitIds);
    case 'resolveBreakTheirWillPick':            return phases.resolveBreakTheirWillPick(G, a.systemId);
    case 'resolveDiscreditRebellion':            return phases.resolveDiscreditRebellion(G, a.action);
    case 'resolveEstablishTradeChoice':          return phases.resolveEstablishTradeChoice(G, a.action);
    case 'resolveFalseOrders':                   return phases.resolveFalseOrders(G, a.targetLeaderId);
    case 'resolveHeistChoice':                   return phases.resolveHeistChoice(G, a.action);
    case 'resolveImperialMightUnits':            return phases.resolveImperialMightUnits(G, a.queueIndices);
    case 'resolveMissionRecruitLeaderPick':      return phases.resolveMissionRecruitLeaderPick(G, a.leaderId);
    case 'resolvePostBountyOffer':               return phases.resolvePostBountyOffer(G, a.leaderId);
    case 'resolvePrepareForBattleDeckPick':      return phases.resolvePrepareForBattleDeckPick(G, a.deckKind);
    case 'resolveRaidOutpostsPlace':             return phases.resolveRaidOutpostsPlace(G, a.systemIds);
    case 'resolveRebelCellDiscard':              return phases.resolveRebelCellDiscard(G, a.objectiveId);
    case 'resolveRebelCellPlace':                return phases.resolveRebelCellPlace(G, a.systemId);
    case 'resolveReconnaissancePick':            return phases.resolveReconnaissancePick(G, a.missionId);
    case 'resolveSabotageChoice':                return phases.resolveSabotageChoice(G, a.action);
    case 'resolveSafeHavenPick':                 return phases.resolveSafeHavenPick(G, a.pickedIndices);
    case 'resolveSecretMissionPick':             return phases.resolveSecretMissionPick(G, a.missionId);
    case 'resolveStartingCardBranch':            return phases.resolveStartingCardBranch(G, a.action);
    case 'resolveTrustInTheForceDestroyPick':    return phases.resolveTrustInTheForceDestroyPick(G, a.instanceId);
    case 'resolveUnderTheRadarKeep':             return phases.resolveUnderTheRadarKeep(G, a.probeId);
    case 'resolveUnderTheRadarReorder':          return phases.resolveUnderTheRadarReorder(G, a.placements);
    case 'resolveUnderTheRadarReturn':           return phases.resolveUnderTheRadarReturn(G, a.accept);
    case 'resolveWereTheBaitUnits':              return phases.resolveWereTheBaitUnits(G, a.unitIds);
    case 'requestImmediateActionCardPlay':       return phases.requestImmediateActionCardPlay(G, actor);
    case 'cancelImmediateActionCardPlay':        return phases.cancelImmediateActionCardPlay(G);
    case 'playImmediateActionCard':              return phases.playImmediateActionCard(G, a.cardId);
    case 'setupUndoDeployUnit':                  return phases.setupUndoDeployUnit(G, actor, a.typeId, a.systemId);
    case 'undoStolenPlansPick':                  return phases.undoStolenPlansPick(G);
    case 'resolveBazesLoyaltyTarget':            return combat.resolveBazesLoyaltyTarget(G, a.instanceId);
    case 'resolveConfrontationLeaderPick':       return combat.resolveConfrontationLeaderPick(G, a.leaderId);
    case 'resolveDsPlansOneInAMillion':          return combat.resolveDsPlansOneInAMillion(G, a.picks);
    case 'resolveDsPlansYoda':                   return combat.resolveDsPlansYoda(G, a.blankIndex);
    case 'resolveRogueOneChoice':                return combat.resolveRogueOneChoice(G, a.action);
    case 'resolveSomethingToFightForOffer':      return combat.resolveSomethingToFightForOffer(G, a.objectiveId);
    case 'resolveTrackThemOffer':                return combat.resolveTrackThemOffer(G, a.leaderId);
    case 'resolveTractorBeamCapturePick':        return combat.resolveTractorBeamCapturePick(G, a.leaderId);
    ...
    case 'declineDeployUnit':                    return phases.declineDeployUnit(G);
    case 'resolveHandLimitDiscard':              return phases.resolveHandLimitDiscard(G, a.missionIds);
    case 'resolveLeaderPoolEliminate':           return phases.resolveLeaderPoolEliminate(G, a.leaderId);
    case 'resolveAssignSecondLeader':            return phases.resolveAssignSecondLeader(G, a.leaderId);
